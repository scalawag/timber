...
/** Provides a [[org.scalawag.timber.api.BaseLogger BaseLogger]] that can be used out-of-the-box with a default set
  ...
  */
class Logger(override val attributes: Map[String, Any] = Map.empty, override val tags: Set[Tag] = Set.empty)(implicit
    dispatcher: Dispatcher
) extends BaseLogger(attributes, tags)(dispatcher)
    with Trace
    with Debug
    with Info
    with Warn
