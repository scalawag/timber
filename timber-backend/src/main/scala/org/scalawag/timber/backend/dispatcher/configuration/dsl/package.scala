...
package object dsl {
  ...
  def logger(attribute:String) = StringConditionFactory(s"""logger("$attribute")""") { entry =>
    for {
  ...
  class ThreadAttribute private[dsl] (attribute: String) {
    ...
    def any = StringConditionFactory(s"""thread("$attribute").any""") { entry =>
    ...
    def top = StringConditionFactory(s"""thread("$attribute").top""") { entry =>
      entry.threadAttributes flatMap { threadAttrs =>
        threadAttrs.get(attribute) flatMap { threadAttrStack =>
  ...
  object choose {
    ...
    def apply(whenThens:choose.WhenThen[MutableVertex]*):Subgraph[MutableVertex] = {
      ...
      whenThens.foldLeft(in:SubgraphWithOutputs[MutableVertexWithOutputs]) { case (input,whenThen) =>
    ...
    final class When(protected[dsl] val whenCondition:Condition) {
      ...
      def ~>[A <: MutableVertexWithOutputs](thenGraph:SubgraphWithOutputs[A]) = new WhenThenWithOutputs(whenCondition,thenGraph)
    }
  }
}
