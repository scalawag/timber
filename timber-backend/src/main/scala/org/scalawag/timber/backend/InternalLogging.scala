...
import org.scalawag.timber.backend.receiver.formatter.{ProgrammableEntryFormatter, EntryFormatter, DefaultEntryFormatter}
...
private[timber] object InternalLogging extends Dispatcher {
  ...
  private[this] val formatter = {
    ...
    if ( debug )
      ...
      new ProgrammableEntryFormatter(metadataProviders = Seq("timber",entry.level formattedWith NameLevelFormatter),
                                     delimiter = ": ",
