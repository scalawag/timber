...
// Just a fake dispatcher that throws an Exception on dispatch so that we can identify it easily.

class ThrowingDispatcher extends Dispatcher {
  override def dispatch(entry: Entry) = throw new Exception("boom")
}

class SpecifiedDispatcherTest extends AnyFunSpec with Matchers {
